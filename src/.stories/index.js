...
storiesOf("Basic settings", module)
  ...
  .add("Disable Specific Dates", () => (
    <InfiniteCalendar
      disabledDates={[-10, -5, -6, 5, 6, 7, 2].map((amount) => addDays(today, amount))}
    />
  ))
  .add("Disable Specific Weekdays", () => <InfiniteCalendar disabledDays={[0, 6]} />)
  .add("With marked dates", () => (
    <InfiniteCalendar
      markedDates={Array.from(new Array(7).keys()).map((index) =>
        format(new Date(new Date().getTime() + ((index - 3) * 2 - 1) * 86400000), "YYYY-MM-DD")
      )}
    />
  ));
...
storiesOf("Customization", module)
  ...
  .add("Flexible Size", () => (
    <InfiniteCalendar width={"94%"} height={window.innerHeight - 147} rowHeight={70} />
  ))
  .add("Select Year First", () => <InfiniteCalendar display={"years"} selected={null} />)
...
storiesOf("Events", module)
  .add("On Select", () => (
    <InfiniteCalendar
      onSelect={(date) => alert(`You selected: ${format(date, "ddd, MMM Do YYYY")}`)}
    />
  ))
  .add("On Scroll", () => [
    ...
    <InfiniteCalendar
      key="calendar"
      onScroll={(scrollTop) => console.info("onScroll() – Scroll top:", scrollTop)}
    />,
  ]);
